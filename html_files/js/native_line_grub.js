...
$(function() {
	...
	GNG.Const.Init=function(){
		...
		var depart_date = parseFloat(new Date(Date.parse(GNG.ReqParam.depart_date)).pattern("MM.dd"));
	...
	GNG.calc_price = function(){
		...
		var singleSeatPrice = 0.0;// 车次中席别的价格，用来计算各席别价格最大值
		GNG.setupPriceTrainInfo = {'trainno' : '', 'seat': '', 'price': 0.0, 'code': ''};// 保存最大价格的车次信息
		
		$.each(GNG.ReqParam.trains, function(i, m){
			$.each(GNG.ReqParam.seats, function(ii, name){
				$.each(proccessYpInfo(m.yp_info ? m.yp_info : m.ypinfo), function(iii, seat){
					if(name == seat.name){
						if(seat.price > singleSeatPrice){
							singleSeatPrice = seat.price;	
							
							GNG.setupPriceTrainInfo.trainno = m.number ? m.number : m.trainno;
							GNG.setupPriceTrainInfo.seat = name;
							GNG.setupPriceTrainInfo.price = singleSeatPrice;
							GNG.setupPriceTrainInfo.code = GNG.Const.native_n2c[name];
						}
					}
				});
			});
		...
		var total_price = GNG.Passenger.GetPassengersForBook().length * singleSeatPrice;
		
		$("#ticket_price").html("￥" + total_price + " [按("+ GNG.setupPriceTrainInfo.trainno + " " + GNG.setupPriceTrainInfo.seat +")预收] ");
	...
	GNG.init = function(){
		NativeAPI.invoke("storage",{action:"get",key:"line_grub"}, function(err, data){
			GNG.ReqParam = JSON.parse(data.value);
			
			var train_nos = [];
			$.each(GNG.ReqParam.trains, function(i, m){
				train_nos[i] = m.number ? m.number : m.trainno;
			});
			GNG.ReqParam.trainno = train_nos.join(",");
			
			var seat_codes = [];
			$.each(GNG.ReqParam.seats, function(i, name){
				seat_codes[seat_codes.length] = GNG.Const.native_n2c[name];
			});
			GNG.ReqParam.seatcodes = seat_codes.join(",");
			
			var tns = train_nos.join("、");
			$("#train_no").html(tns.length > 14 ? (tns.substring(0, 11) + "...") : tns);
			$("#depart_date").html(GNG.ReqParam.depart_date);
			var sns = GNG.ReqParam.seats.join("、");
			$("#train_seats").html(sns.length > 11 ? (sns.substring(0, 8) + "...") : sns);
			$("#train_depart_times").html(GNG.ReqParam.depart_time_area.left + " ~ " + GNG.ReqParam.depart_time_area.right);
			$("#depart_city").html(GNG.ReqParam["depart_name"]);
			$("#arrive_city").html(GNG.ReqParam["arrive_name"]);
			
			GNG.ReqParam.depart_time = GNG.ReqParam.depart_time_area.right;
			GNG.Time.Init();
			GNG.AnaGrub();
			GNG.Notify.Init();
			
			if(GNG.ReqParam.seats.join(",").indexOf('硬座') > 0 || GNG.ReqParam.seats.join(",").indexOf('二等座') > 0){
	...
	GNG.Time.Init=function(){
		var depart_date = Date.parse(GNG.ReqParam.depart_date);
		var today = Date.parse(new Date().pattern('yyyy-MM-dd'));
		var nowHour = new Date().getHours();
		var depart_hour = 24;// 线路抢票可以设置为24，服务器控制是否买票
...
*/
	...
	GNG.Time.GetTimeStr=function(){
		...
		if('开车前' == expiredate){
			...
			return new Date(Date.parse(GNG.ReqParam.depart_date)).pattern("M月d日") + " " + expiretime + ":" + GNG.ReqParam.depart_time.split(':')[1];
		}
	...
	GNG.Passenger.add=function(p){
		...
		html = html.replace(/_seat_type_/, GNG.ReqParam.seats.join("、"));
	...
	GNG.Passenger.GetPassengersForBook=function(){
		...
		$("#passengers_hidden").find("div[class='clickable_passenger']").each(function(){
			...
			p[1] = GNG.setupPriceTrainInfo ? GNG.setupPriceTrainInfo.code : "";
			p[2] = GNG.setupPriceTrainInfo ? GNG.setupPriceTrainInfo.price : "";
			str[i++] = p.join(",");
		});
		return str;
	}
	...
	GNG.AnaGrub.SetUp=function(ins){
		var level = 0;
		$("#grub_line_count").html(GNG.ReqParam.trains.length);
		if(GNG.ReqParam.trains.length < 5){
			level++;
		}
								...
								NativeAPI.invoke("get12306UserInfo", {}, function(err, data) {
									...
									$.ajax({
										...
										data : {
											...
											"p":p,
											"traintype": "3",
											"trains": GNG.ReqParam["trainno"],
											"seattype": '1',
											"seats": GNG.ReqParam["seatcodes"],
											"account":username,
											"password":password,
											"acfcity":"1",
											"actcity":"1",
											"departdate": GNG.ReqParam["depart_date"],
											...
											"trainstart":GNG.ReqParam.depart_time_area.left,
											"trainend":GNG.ReqParam.depart_time_area.right,
